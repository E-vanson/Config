...
describe('Target: % of Children with Pneumonia Treated', () => {
  before(async () => {
  ...
  it('default target value', async () => {
    const [analytics] = await harness.getTargets({ type: Targets.U5_WITH_PNEUMONIA_TREATED });
    expect(analytics).to.nested.include({ 'value.pass': 0, 'value.total': 0, 'value.percent': 0 });
  });

  it('target value.pass updates when given amox', async () => {
  ...
  it('target value resets monthly', async () => {
    ...
    const result = await harness.fillForm(Services.U5_ASSESSMENT, ...u5AssessmentScenarios.over2Month_fast_breathing);
    expect(result.errors).to.be.empty;
    let [analytics] = await harness.getTargets({ type: Targets.U5_WITH_PNEUMONIA_TREATED });
    expect(analytics).to.nested.include({ 'value.pass': 1, 'value.total': 1, 'value.percent': 100 });

    harness.setNow(DateTime.now().endOf('month').plus({ day: 7 }).toISODate());
    [analytics] = await harness.getTargets({ type: Targets.U5_WITH_PNEUMONIA_TREATED });
    expect(analytics).to.nested.include({ 'value.pass': 0, 'value.total': 0, 'value.percent': 0 });
  });
  ...
  it('target value.pass does not update when not given amox', async () => {
    harness.subject.date_of_birth = DateTime.now().minus({ year: 3 }).toISODate();
    const result = await harness.fillForm(Services.U5_ASSESSMENT, ...u5AssessmentScenarios.over2Month_fast_breathing_not_given_amox);
    expect(result.errors).to.be.empty;
    const [analytics] = await harness.getTargets({ type: Targets.U5_WITH_PNEUMONIA_TREATED });
